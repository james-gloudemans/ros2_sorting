...
class Event:
    ...
    def L2_released(self):
        if not self.connecting_using_ds4drv:
            return (
                self.button_id == 2 and self.button_type == 2 and self.value == -32767
            )
    ...
    def R2_released(self):
        if not self.connecting_using_ds4drv:
            return (
                self.button_id == 5 and self.button_type == 2 and self.value == -32767
            )
    ...
    def listen(self, timeout=30, on_connect=None, on_disconnect=None, on_sequence=None):
        ...
        try:
            ...
            while not self.stop and event:
                (*tv_sec, value, button_type, button_id) = struct.unpack(
                    self.event_format, event
                )
                if self.debug:
                    print(
                        "button_id: {} button_type: {} value: {}".format(
                            button_id, button_type, value
                        )
                    )
                if button_id not in self.black_listed_buttons:
                    self.__handle_event(
                        button_id=button_id, button_type=button_type, value=value
                    )
